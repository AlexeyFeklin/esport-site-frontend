...
import styles from './NewsZoneCart.module.css';
import { useDispatch } from 'react-redux';
import { removeNews } from '../../redux/slices/news';
import notPhoto from './../../assets/img/camera_yqpp2gkt93iq.svg';

export const NewsZoneCart = ({ user, news }) => {
  ...
  return (
    <Link
      to={news?.isAddBlock ? '/admin/news/create' : '/admin/news/' + news._id}
      ...
      <div className={styles.roleApplication_Cart_info}>
        ...
        <div className={styles.zone_text}>
          <div className={styles.player_name}>
            {news?.isAddBlock ? 'Добавить новую новость' : news?.title}
          </div>
          {news?.isAddBlock ? (
            ''
          ) : (
      ...
      {news?.isAddBlock ? (
        ...
        <div
          ...
          <svg
            onClick={() =>
              window.confirm('Вы действительно хотите удалить новость + ' + news._id + '?')
